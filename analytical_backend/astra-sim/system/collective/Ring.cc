...
Ring::Ring(
  ...
  switch (type) {
    case ComType::All_Reduce:
      stream_count = 2 * (nodes_in_ring - 1); // the number of hops with unidirectional communication, so stream here indicates one-hop communication unit 
      break;
    case ComType::All_to_All:
      this->stream_count = ((nodes_in_ring - 1) * nodes_in_ring) / 2; // the number of hops with unidirectional communication 
      ...
      break;
    default:
      stream_count = nodes_in_ring - 1; // for Reduce-Scatter and All-Gather
  }
