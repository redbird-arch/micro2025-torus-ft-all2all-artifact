...
SYSTEM_DIR=${SCRIPT_DIR}/../../../../inputs/system/Synthetic_exp/MATE
WORKLOAD_DIR=${SCRIPT_DIR}/../../../../inputs/workload/Scalability_exp/Weak
STATS_DIR=${SCRIPT_DIR}/../../../../examples/results/Scalability_exp/Algorithm_weak/MATE
...
for system_file in ${SYSTEM_DIR}/*.txt; do
    if [[ -f "$system_file" ]]; then
        total_experiments=$(($total_experiments + 9))  # Each system configuration matches 7 network files
    fi
done
