...
SYSTEM_DIR=${SCRIPT_DIR}/../../../inputs/system/Synthetic_exp/Ring_No_Fault_Pipeline
WORKLOAD_DIR=${SCRIPT_DIR}/../../../inputs/workload/Real_Model
STATS_DIR=${SCRIPT_DIR}/../../../examples/results/Real_Model/No_Fault_Ring
...
for workload_file in ${WORKLOAD_DIR}/*_TPUv*.txt; do
    ...
    network_count=$(ls ${NETWORK_DIR}/${network_version}_NoFault.json | wc -l)
...
for workload_file in ${WORKLOAD_DIR}/*_TPUv*.txt; do
    ...
    if [[ -f "${system_file}" ]]; then
        ...
        # Loop through all matching network files, specifically the 'NoFault' files
        for network_file in ${NETWORK_DIR}/${network_version}_NoFault.json; do
            if [[ -f "${network_file}" ]]; then
                ...
                "${BINARY}" \
                  --run-name="${workload%.txt}_NoFault" \
