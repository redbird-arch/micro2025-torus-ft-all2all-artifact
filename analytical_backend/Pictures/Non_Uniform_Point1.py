...
# === Path ===
file_no_fault = "../examples/results/Non_Uniform_MoE/No_Fault/EndToEnd.csv"
dor_base_dir = "../../garnet_backend/examples/results/Non_Uniform_MoE/No_Fault"
...
# Uniform_DOR = extract_wg_exposed_comm(file_mate)
Uniform_DOR = extract_mate_wg_exposed_comm(dor_base_dir)
...
colors_mate, norm_mate, cmap_mate = generate_colormap(Uniform_DOR, 'YlOrRd', 0.05, 0.9)

# === get the figure ===
fig, ax = plt.subplots(figsize=(21, 12))
sc_nf = ax.scatter(layers, Uniform_No_Fault[-32:], color=colors_nf[-32:], s=2400, marker='o', edgecolors='black', linewidths=4)
sc_mate = ax.scatter(layers, Uniform_DOR[-32:], color=colors_mate[-32:], s=2400, marker='p', edgecolors='black', linewidths=4)
# add two special points after scatter-point-figure (uniform)
ax.scatter(0, Uniform_No_Fault[0], color='#80ed99', s=5400, marker='*', edgecolors='#06d6a0', linewidths=7, zorder=5)
ax.scatter(0, Uniform_DOR[0], color='#ffc971',  s=5400, marker='*', edgecolors='#f2a65a', linewidths=7, zorder=5)
...
ax.set_ylim(3.8, 6.9)
ax.set_yticks(np.arange(4, 6.9, 0.5))
...
cbar_nf.set_label("HalfR+DR", labelpad=12, fontsize=48)
...
cbar_mate.set_label("DOR", labelpad=12, fontsize=48)
cbar_mate.ax.tick_params(labelsize=42, pad=5)

plt.subplots_adjust(right=0.86, bottom=0.13)  
plt.savefig("Non_Uniform_Point1.png", dpi=600)
plt.savefig("Non_Uniform_Point1.pdf")
# plt.show()


print("=== The time proportion of Uniform_HalfRing+DR compared with the first element of Uniform (index 0) ===")
...
print("=== The time proportion of Uniform_DOR compared with the first element of Uniform (index 0) ===")
base_uniform = Uniform_DOR[0]
ratios_to_uniform2 = [v / base_uniform for v in Uniform_DOR]
for idx, ratio in enumerate(ratios_to_uniform):
    print(f"Layer {idx}: {ratio:.3f}x")
...
print("\n=== Uniform_DOR vs. Uniform_HalfRing+DR the time radio of each layer(index 1~32) ===")
mate_ratios = []
for i in range(1, min(len(Uniform_DOR), len(Uniform_No_Fault))):
    if Uniform_No_Fault[i] != 0:
        ratio = Uniform_DOR[i] / Uniform_No_Fault[i]
        mate_ratios.append(ratio)
        print(f"Layer {i}: MATE = {Uniform_DOR[i]:.1f}, No_Fault = {Uniform_No_Fault[i]:.1f}, Ratio = {ratio:.3f}x")
    else:
        print(f"Layer {i}: No_Fault time is zero, skipped.")

if Uniform_No_Fault[0] != 0:
    first_ratio = Uniform_DOR[0] / Uniform_No_Fault[0]
    print(f"\n→ Uniform MATE = {Uniform_DOR[0]:.1f}, No_Fault = {Uniform_No_Fault[0]:.1f}, Ratio = {first_ratio:.3f}x")
else:
