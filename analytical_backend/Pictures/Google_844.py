...
def compute_bandwidth_with_mask(time_list):
    ...
    Compute a bandwidth array (GB/s) from time_list (in microseconds):
    size = 64KB * 2**i; bw = size / time / 1000.
    Returns a numpy array and a boolean mask for zeros.
    """
    bw = []
    zero_mask = []
    for i, t in enumerate(time_list):
...
# ===== 1. Read HalfRing_DimRotation data (8x4x4 NoFault) =====
half_csv = os.path.join(
    '..', 'examples', 'results', 'Google_844',
    'TPUv4_8x4x4_NoFault_No_Fault', 'EndToEnd.csv'
)
HalfRing_times = read_series_from_fault_tolerance_csv(half_csv)

# ===== 2. Read MATE and MATEe data (8x4x4 SingleFault) =====
mate_csv = os.path.join(
    '..', 'examples', 'results', 'Google_844',
    'TPUv4_8x4x4_SingleFault_MATE', 'EndToEnd.csv'
)
matee_csv = os.path.join(
    '..', 'examples', 'results', 'Google_844',
    'TPUv4_8x4x4_SingleFault_MATEe', 'EndToEnd.csv'
)
...
# ===== 6. Compute average speedups (skip zero denominators) =====
...
speedup_MATE_WFR,        valid2 = safe_speedup(MATE, WFR_2)
speedup_MATEe_WFR,       valid3 = safe_speedup(MATEe, WFR_2)
slowdown_WFR_DORMIN,     valid4 = safe_speedup(WFR_2, DORMIN)

print("Average speedup HalfRing vs DORMIN:", gmean(valid1) if len(valid1) else 0.0)
print("Average speedup MATE vs WFR_2:",      gmean(valid2) if len(valid2) else 0.0)
print("Average speedup MATEe vs WFR_2:",     gmean(valid3) if len(valid3) else 0.0)
print("Average slowdown WFR_2 vs DORMIN:",   gmean(valid4) if len(valid4) else 0.0)
...
legends     = ['DOR', 'HalfR+DR', 'WFR-F1', 'WFR-F2', 'MATE', 'MATEe']
fill_colors = ['#80ed99', '#4cc9f0', '#ffc971', '#db7c26', '#ff87ab', '#c77dff']
line_colors = ['#06d6a0', '#118ab2', '#f2a65a', '#d8572a', '#ff5d8f', '#7b2cbf']
linestyles  = ['-'] * 6
markers     = ['o', 'D', 's', 'p', '^', 'v']
data_arrays = [DORMIN, HalfRing_DimRotation, WFR_1, WFR_2, MATE, MATEe]
zero_masks  = [DORMIN_zero_mask, None, WFR_1_zero_mask, WFR_2_zero_mask, None, None]
...
ax.set_xlabel('All-to-All Size on 8x4x4 Torus', fontsize=55)
...
ax.set_ylim(15, 63)
ax.set_yticks(np.arange(20, 63, 10))
ax.grid(True, linewidth=1.4)
ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.24),
          handletextpad=0.25, ncol=3, fontsize=44,
          markerscale=1.2, borderpad=0.25, columnspacing=3.0)
plt.subplots_adjust(top=0.87, bottom=0.20)
plt.savefig('Google_844_IBM.pdf')
plt.savefig('Google_844_IBM.png', dpi=400)