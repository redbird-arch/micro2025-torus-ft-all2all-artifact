...
#include "astra-sim/system/scheduling/OneStage.hh"
...
Sys::Sys(
  ...
  if (inter_dimension_scheduling == InterDimensionScheduling::OneStage) {
    one_stage = new OneStage(this);
  }
}
...
bool Sys::parse_var(std::string var, std::string value) {
  ...
  if (var == "scheduling-policy:") {
    ...
    } else if (tmp == "onestage") {
      inter_dimension_scheduling = InterDimensionScheduling::OneStage;
    } else {
      sys_panic(
  ...
  while (size > 0) {
    ...
    if (inter_dimension_scheduling == InterDimensionScheduling::RoundRobin) {
      ...
      chunk_size = prev_size - size;
        ...
        collective_type == ComType::All_to_All &&
        (inter_dimension_scheduling == 
         InterDimensionScheduling::OneStage)){
        dim_mapper = one_stage->get_chunk_scheduling();
    }
